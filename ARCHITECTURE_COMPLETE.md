...
│  ├── device_verification_page.dart        └── biometric_service.dart         │
│  └── device_approval_page.dart                                               │
│  models/                                                                     │
...
│  POST /api/auth/verify-otp              /var/www/trusted-devices/{phone}.json│
│  POST /api/auth/reset-pin               /var/www/device-approval-requests/  │
│  POST /api/auth/validate-session                                             │
│  POST /api/auth/logout                                                       │
│  POST /api/auth/verify-device-otp        ← NEW: Привязка устройства        │
│  POST /api/auth/request-device-approval  ← NEW: Запрос одобрения           │
│  GET  /api/auth/device-approval-requests ← NEW: Список запросов (dev)      │
│  POST /api/auth/resolve-device-approval  ← NEW: Одобрить/отклонить         │
│  GET  /api/auth/device-approval-status   ← NEW: Статус запроса (polling)   │
│                                                                              │
└─────────────────────────────────────────────────────────────────────────────┘
                                    │
                                    │ Telegram API
...
## 3.5 Привязка к устройству (Device Binding)

**Цель:** Предотвратить вход в чужой аккаунт даже при знании PIN.

**Принцип:** Каждый пользователь привязан к одному физическому устройству. Вход с нового устройства требует верификации.

**Feature flag:** `DEVICE_BINDING_ENABLED=true/false` в pm2 env. При `false` — старое поведение (только PIN).

**Таблицы:** `trusted_devices` (привязка phone→device_id), `device_approval_requests` (очередь запросов).

**Flow при входе с нового устройства:**
1. Пользователь вводит PIN → сервер отвечает 403 `NEW_DEVICE_DETECTED`
2. Flutter показывает `DeviceVerificationPage` с двумя вариантами:
   - **Telegram OTP**: запрос кода → подтверждение → устройство доверено
   - **Запрос разработчику**: push-уведомление → разработчик одобряет в `DeviceApprovalPage`
3. После верификации устройство записывается в `trusted_devices`

**Миграция:** Первый вход после включения флага автоматически доверяет текущее устройство (`trusted_via: 'migration'`).

**Откат:** `DEVICE_BINDING_ENABLED=false` → перезапуск → мгновенный откат без потери данных.

## 3.6 Параметры безопасности
...
| Привязка к устройству | DEVICE_BINDING_ENABLED | auth_api.js (pm2 env) |
...
messenger/
├── models/
│   ├── chat_folder_model.dart        # Модель папки чатов
...
│   ├── messenger_chat_page.dart      # Экран чата (O(1) reply, in-place reactions)
...
│   ├── conversation_picker_page.dart # Выбор беседы (пересылка)
...
└── widgets/
    ...
    ├── message_context_menu.dart     # Контекстное меню сообщения
    ├── message_input_bar.dart        # Панель ввода с голосовыми
    ├── pinned_message_bar.dart       # Панель закреплённых сообщений
...
**Статус:** Активный (замена employee_chat)

**Файлов:** 36 Flutter + 3 бэкенд = 39
...
- **Видеосообщения** — запись (video_note_recorder_page) + воспроизведение (video_note_player)
- **Аудиозвонки** — WebRTC через STUN-серверы (call_service + call_page)
- **Shell навигация** — `messenger_shell_page.dart` как обёртка
- **Группы и каналы** — создание, управление участниками, информация
- **Опросы** — создание и голосование (poll_model + create_poll_page)
- **Папки чатов** — организация бесед (chat_folder_model + manage_folders_page)
- **Стикеры и GIF** — выбор и отправка
- **Профили** — страница профиля пользователя
- **Rate limiting** — 30 сообщений/мин и 10 загрузок/мин на пользователя
- **Денормализованный unread_count** — быстрый подсчёт непрочитанных
- **Автоочистка медиа** — scheduler удаляет файлы старше 90 дней

**API Endpoints:**

| Method | Path | Описание |
|--------|------|----------|
| GET | `/api/messenger/conversations` | Список бесед (LATERAL JOIN + batch participants) |
| POST | `/api/messenger/conversations` | Создать беседу |
| GET | `/api/messenger/conversations/:id/messages` | Сообщения беседы |
| POST | `/api/messenger/conversations/:id/messages` | Отправить сообщение (rate limit 30/мин) |
| POST | `/api/messenger/conversations/:id/read` | Прочитать сообщения |
| GET | `/api/messenger/unread` | Общее число непрочитанных |
| GET | `/api/messenger/contacts` | Список контактов |
| POST | `/api/messenger/upload` | Загрузка медиа (rate limit 10/мин) |
| POST | `/api/messenger/groups` | Создать группу |
| PUT | `/api/messenger/groups/:id` | Обновить группу |
| GET | `/api/messenger/media-stats` | Статистика медиа (dev only) |
...
| POST | `/api/auth/verify-device-otp` | Подтвердить устройство через OTP | ❌ |
| POST | `/api/auth/request-device-approval` | Запрос на одобрение устройства | ❌ |
| GET | `/api/auth/device-approval-requests` | Список ожидающих запросов | ✅ (dev) |
| POST | `/api/auth/resolve-device-approval` | Одобрить/отклонить запрос | ✅ (dev) |
| GET | `/api/auth/device-approval-status/:phone` | Статус запроса (polling) | ❌ |
...
| 12 | Messenger Media Cleanup | `messenger_media_cleanup_scheduler.js` | ежедневно 4:00 МСК | N/A | N/A | N/A |

> **Messenger Media Cleanup** — удаляет медиафайлы мессенджера старше 90 дней (настраивается через MESSENGER_MEDIA_RETENTION_DAYS), если файл не привязан к живому сообщению. Первые 5 запусков — dry run (только логирование). Включается через `MESSENGER_MEDIA_CLEANUP_ENABLED=true`.
...
| `auth_sessions` | id (SERIAL) | Сессии (unique on phone, +device_id) |
| `auth_pins` | phone | PIN-коды |
| `trusted_devices` | phone | Привязка устройства к пользователю |
| `device_approval_requests` | id | Запросы на одобрение нового устройства |
...
| `messenger_conversations` | id | Беседы мессенджера |
| `messenger_participants` | (conv_id, phone) | Участники бесед (+unread_count) |
| `messenger_messages` | id | Сообщения мессенджера |
| `messenger_profiles` | phone | Профили пользователей мессенджера |
...
├── messenger-media/               # Медиа мессенджера (фото, видео, голосовые, файлы)
│                                  # Автоочистка: messenger_media_cleanup_scheduler (90 дней)
│
│ ─── ИИ РАСПОЗНАВАНИЕ ───
