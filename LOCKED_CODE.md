...
| 26.01.2026 | v1.6.2 | Тестирование: автоматическое начисление баллов, UI с анимацией, линейная интерполяция, настройки |
...
## Тестирование (v1.6.2) - Проверено: 26.01.2026

### Общее описание:
Система тестирования сотрудников с автоматическим начислением баллов на основе результатов. Баллы рассчитываются по алгоритму линейной интерполяции и интегрируются с модулем Эффективность. Результаты отображаются с анимацией сразу после завершения теста.

---

### Функционал:
- 20 вопросов с 4 вариантами ответа
- Таймер прохождения теста
- Автоматическое начисление баллов сразу при сохранении результата
- Линейная интерполяция баллов (minPoints → 0 → maxPoints)
- Анимация отображения баллов (elasticOut) с цветовой кодировкой
- Русское склонение слова "балл" (1 балл, 2 балла, 5 баллов)
- Настройки баллов для админа (минимум, максимум, порог нуля)
- Интеграция с модулем Эффективность
- Дедупликация начисления через sourceId
...
| Файл | Функционал | Статус |
|------|------------|--------|
| `lib/features/tests/pages/test_page.dart` | Прохождение теста, таймер, анимация баллов, склонение слов | ✅ Работает |

#### Страницы (Админ)
| Файл | Функционал | Статус |
|------|------------|--------|
| `lib/features/tests/pages/test_results_page.dart` | Просмотр всех результатов тестирования | ✅ Работает |
| `lib/features/tests/pages/test_questions_management_page.dart` | Управление вопросами теста | ✅ Работает |
| `lib/features/efficiency/pages/settings_tabs/test_points_settings_page.dart` | Настройки баллов (минимум, максимум, порог нуля) | ✅ Работает |

#### Модели
| Файл | Функционал | Статус |
|------|------------|--------|
| `lib/features/tests/models/test_result_model.dart` | TestResult: id, score, totalQuestions, timeSpent, points, shopAddress | ✅ Работает |
| `lib/features/tests/models/test_question_model.dart` | TestQuestion: question, options, correctAnswer | ✅ Работает |

#### Сервисы
| Файл | Функционал | Статус |
|------|------------|--------|
| `lib/features/tests/services/test_result_service.dart` | getResults, saveResult (возвращает TestResult с баллами) | ✅ Работает |
| `lib/features/tests/services/test_question_service.dart` | getQuestions | ✅ Работает |
...
| Endpoint | Метод | Статус |
|----------|-------|--------|
| `/api/test-questions` | GET | ✅ Получить все вопросы |
| `/api/test-questions` | POST | ✅ Создать вопрос |
| `/api/test-questions/:id` | PUT | ✅ Обновить вопрос |
| `/api/test-questions/:id` | DELETE | ✅ Удалить вопрос |

#### Результаты
| Endpoint | Метод | Статус |
|----------|-------|--------|
| `/api/test-results` | GET | ✅ Получить все результаты |
| `/api/test-results` | POST | ✅ Сохранить результат (с начислением баллов) |

#### Настройки баллов
| Endpoint | Метод | Статус |
|----------|-------|--------|
| `/api/points-settings/test` | GET | ✅ Получить настройки |
| `/api/points-settings/test` | POST | ✅ Сохранить настройки |
...
| Файл | Функционал | Статус |
|------|------------|--------|
| `loyalty-proxy/index.js` | POST /api/test-results → assignTestPoints() с линейной интерполяцией | ✅ Работает |
...
| Директория | Содержимое |
|------------|------------|
| `/var/www/test-questions/` | JSON файлы вопросов теста |
| `/var/www/test-results/` | JSON файлы результатов тестирования |
| `/var/www/efficiency-penalties/{YYYY-MM}.json` | Баллы за тесты (интеграция с Эффективностью) |
| `/var/www/points-settings/test_points_settings.json` | Настройки баллов |
...
| Параметр | По умолчанию | Описание |
|----------|--------------|----------|
| `minPoints` | -3.0 | Баллы за 0% правильных ответов |
| `maxPoints` | +3.5 | Баллы за 100% правильных ответов |
| `zeroThreshold` | 15 | Количество правильных ответов для 0 баллов |

---

### Алгоритм начисления баллов (линейная интерполяция):

#### Функция assignTestPoints() (в loyalty-proxy/index.js)
```javascript
// 1. Если score <= 0:
//    points = minPoints (например, -3)

// 2. Если score >= totalQuestions:
//    points = maxPoints (например, +3.5)

// 3. Если score <= zeroThreshold (например, 15):
...
//    → range = 20 - 15 = 5
//    → points = 3.5 × ((18 - 15) / 5) = 3.5 × 0.6 = 2.1
...
#### Анимация баллов (test_page.dart)
```dart
// AnimationController с длительностью 600ms
// Кривая анимации: Curves.elasticOut (эффект "отскока")
// ScaleTransition: scale от 0.0 до 1.0

// Цветовая кодировка:
// - Положительные баллы: зелёный (#4CAF50)
// - Отрицательные баллы: красный (#F44336)

// Иконки:
// - Положительные: Icons.add_circle
// - Отрицательные: Icons.remove_circle
```
...
String _getBallsWordForm(double points) {
  ...
  } else if ([2, 3, 4].contains(absPoints % 10) &&
             ![12, 13, 14].contains(absPoints % 100)) {
...
Сотрудник проходит тест
    ↓
Результат сохраняется через POST /api/test-results
    ↓
Сервер вызывает assignTestPoints():
  1. Загружает настройки из test_points_settings.json
  2. Рассчитывает баллы по формуле линейной интерполяции
  3. Проверяет дедупликацию (sourceId)
  4. Создаёт запись в /var/www/efficiency-penalties/{YYYY-MM}.json
  5. Возвращает TestResult с начисленными баллами
    ↓
Клиент получает TestResult с полем "points"
    ↓
UI отображает баллы с анимацией elasticOut
```
...
**Результаты:**
| Правильных | Процент | Формула | Баллы |
|------------|---------|---------|-------|
| 0 | 0% | minPoints | -3.0 |
| 7 | 35% | -3 + 3 × (7/15) | -1.6 |
| 10 | 50% | -3 + 3 × (10/15) | -1.0 |
...
1. **Синхронное начисление баллов:**
   - Баллы начисляются сразу при сохранении результата
   - Не требуется scheduler (в отличие от product_questions)
   - TestResult возвращается с полем `points`
