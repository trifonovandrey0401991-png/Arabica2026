...
- `v1.0-network-fix` - **Исправлена проблема с подключением Android эмулятора к серверу**
  - Открыты порты 80 (HTTP) и 443 (HTTPS) в файрволе ufw
  - Добавлен IP адрес сервера (62.113.41.32) в network_security_config.xml
  - Добавлено дополнительное логирование для диагностики сетевых проблем
  - Сервер теперь доступен извне, Android эмулятор может подключаться
  - Google Apps Script работает корректно (3-4 секунды ответа)
  - Дата: 2025-12-03
