...
> Дата обновления: 2026-02-17 (верифицировано по реальной файловой структуре)
...
│   ├── widgets/ (1 файл)             │
│   └── utils/ (5 файлов)             ├── utils/ (8 файлов)
├── features/ (35 модулей, ~409 dart) ├── services/ (1 файл: telegram)
├── shared/ (14 файлов)               └── /var/www/ (110+ директорий JSON)
└── app/ (9 файлов)
```

**Ключевые числа:**
- 35 Flutter модулей (~409 .dart файлов в features/)
...
- 8 утилит бэкенда (utils/): кэш, пагинация, сессии, файловые операции
- 6 ролей пользователей
- Файловое JSON-хранилище (без БД), 110+ директорий в /var/www/
...
| **work_schedule** | `features/work_schedule/` | 11 | BaseHttpService, ApiConstants |
| **shift_handover** | `features/shift_handover/` | 13 | BaseHttpService, ApiConstants, PhotoUploadService, MultitenancyFilterService, EmployeePushService |

### Отчёты и инвентаризация
| Модуль | Папка | Файлов | Зависимости Core |
|--------|-------|--------|-----------------|
| **recount** | `features/recount/` | 20 | BaseHttpService, ApiConstants, PhotoUploadService, MultitenancyFilterService, EmployeePushService |
...
| **efficiency** | `features/efficiency/` | 57 | BaseHttpService, ApiConstants, CacheManager, MultitenancyFilterService |
...
| **ai_training** | `features/ai_training/` | 27 | BaseHttpService, ApiConstants |
...
### WORK_SCHEDULE (Рабочий график) — 11 файлов
...
### EFFICIENCY (Эффективность) — 57 файлов
...
### AI_TRAINING (ИИ обучение / Vision) — 27 файлов
...
### Утилиты бэкенда (utils/) — 8 файлов

| Файл | Назначение |
|------|-----------|
| `utils/admin_cache.js` | Кэш админских данных (preload + periodic rebuild) |
| `utils/async_fs.js` | Асинхронные файловые операции |
| `utils/data_cache.js` | Общий кэш данных |
| `utils/file_helpers.js` | Хелперы файлов (maskPhone и др.) |
| `utils/file_lock.js` | Блокировка файлов при записи |
| `utils/test_file_lock.js` | Тест блокировки |
| `utils/pagination.js` | Пагинация ответов |
| `utils/session_middleware.js` | Session middleware (JWT) |
...
### Хранилище данных (/var/www/) — 110+ директорий
...
### shared/ — Общие виджеты и модели (14 файлов)
...
Используется в: work_schedule (dialogs), orders (providers), employee_chat/clients (widgets)

### app/ — Навигация и общие страницы (9 файлов)
