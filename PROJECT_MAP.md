...
> Дата обновления: 2026-02-24 (верифицировано по реальной файловой структуре)
...
│   ├── constants/ (3 файла)          ├── api/ (74 файла)
...
├── features/ (37 модулей, 454 dart)  ├── PostgreSQL (arabica_db, ~44 таблиц)
├── shared/ (18 файлов)               └── /var/www/ (110+ директорий JSON, backup)
└── app/ (10 файлов)
```

**Ключевые числа:**
- 37 Flutter модулей (454 .dart файлов в features/, ~509 всего в lib/)
- 74 API файла бэкенда (api/)
...
|--------|-------|----------------|---------------|
| **BaseHttpService** | `core/services/base_http_service.dart` | ВСЕ 36 модулей | Сломается ВСЁ |
...
| **ai_training** | `features/ai_training/` | 33 | BaseHttpService, ApiConstants |
...
| Файл | Влияние |
|------|---------|
| `base_http_service.dart` | ВСЕ 36 модулей |
...
| `index.js` (бэкенд роутер) | ВСЕ API эндпоинты (71 файл) |
...
```
Подключает 74 API файла через require() и setup*()
...
WebSocket: employee_chat_websocket + messenger_websocket
```
...
### Полный список API файлов (74 штуки)
...
**AI/Vision (8):** cigarette_vision_api, z_report_api, shift_ai_verification_api, master_catalog_api, master_catalog_notifications, ai_dashboard_api, yolo_retrain_scheduler
