...
├── pages/registration_page.dart          ← SingleChildScrollView layout, логотип в золотой рамке
└── pages/pin_page.dart
...
Бэкенд push-уведомления (modules/orders.js):
├── type: 'new_order'    → админам при создании заказа
└── type: 'order_status' → клиенту при принятии/отклонении

Зависит от: BaseHttpService, BonusService (списание бонусов)
От него зависят: efficiency (категория заказы), loyalty (бонусы от заказов), firebase_service.dart (навигация по push)

⚠️ ЕСЛИ ИЗМЕНИТЬ ORDERS:
→ Efficiency потеряет категорию "Заказы"
→ Бонусы при заказе перестанут начисляться
→ Push-навигация (firebase_service.dart) зависит от значений type
```
...
| `firebase_service.dart` | orders (навигация по push), clients (регистрация FCM), + зависит от UserRoleService |
...
PUBLIC_WRITE_PATHS: ['/', '/api/auth', '/api/fcm-tokens', ...] — эндпоинты без авторизации
⚠️ Если добавить/убрать require — весь сервер может не запуститься
⚠️ Если убрать путь из PUBLIC_WRITE_PATHS — этот эндпоинт станет требовать авторизацию
```
