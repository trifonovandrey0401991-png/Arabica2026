...
### MESSENGER (Мессенджер) — 17 файлов
...
Зависит от: BaseHttpService, ApiConstants, WebSocket
От него зависят: (изолированный модуль, но аналогичен employee_chat)

ЕСЛИ ИЗМЕНИТЬ MESSENGER:
> WebSocket может разорваться — сообщения не приходят в реальном времени
> Голосовые сообщения могут не записываться/не отправляться
```
...
```
Подключает 75 API файлов через require() и setup*()
50+ вызовов setup*API() для маршрутизации
11 планировщиков запущены автоматически
...
### Планировщики (Schedulers) — 11 штук, ВСЕ в api/
