...
> Дата обновления: 2026-03-04 (верифицировано по реальной файловой структуре)
...
│   ├── constants/ (4 файла)          ├── api/ (77 файлов: 64 обработчика + 10 планировщиков + 3 WebSocket)
│   ├── services/ (16 файлов)         ├── modules/ (9 файлов: OCR, vision, orders, z-report intelligence)
│   ├── theme/ (1 файл)               ├── ml/ (5 файлов: YOLO inference, server, wrapper, embeddings, build catalog)
│   ├── widgets/ (1 файл)             ├── utils/ (18 файлов, вкл. db.js, push_service)
│   └── utils/ (6 файлов)             ├── services/ (1 файл: telegram)
├── features/ (39 модулей, 482 dart)  ├── PostgreSQL (arabica_db, ~44 таблиц)
├── shared/ (18 файлов)               └── /var/www/ (113+ директорий JSON, backup)
└── app/ (12 файлов)
```

**Ключевые числа:**
- 39 Flutter модулей (482 .dart файлов в features/, 541 всего в lib/)
- 77 API файлов бэкенда (api/): 64 обработчика + 10 планировщиков + 3 WebSocket
- 12 автоматических планировщиков (schedulers) — ВСЕ в api/, НЕ в modules/
- 9 модулей бэкенда (modules/): OCR, vision, orders, z-report intelligence
- 5 ML файлов (ml/): YOLO inference, server, wrapper, embedding catalog, build reference catalog
- 18 утилит бэкенда (utils/): кэш, пагинация, сессии, файловые операции, db.js, moscow_time, push_service, pending_notify и др.
- 6 ролей пользователей
- PostgreSQL (arabica_db, ~44 таблицы, 10400+ записей) + JSON файлы как backup/fallback
- 47 feature flag (`USE_DB_*=true`) для переключения между PostgreSQL и JSON
...
> Сломается ВСЁ приложение (все 36 модулей)
...
| `core/constants/prefs_keys.dart` | Ключи SharedPreferences |

### 2.4 Core сервисы (16 файлов)

| Сервис | Файл | Кто использует | Если изменить |
|--------|-------|----------------|---------------|
| **BaseHttpService** | `core/services/base_http_service.dart` | ВСЕ 38 модулей | Сломается ВСЁ |
...
| **DiskCache** | `core/utils/disk_cache.dart` | efficiency, kpi | Офлайн кэш на диске |
...
|--------|-------|--------|-----------------|
| **auth** | `features/auth/` | 15 | BaseHttpService, ApiConstants, SharedPreferences |
...
| **shop_catalog** | `features/shop_catalog/` | 6 | BaseHttpService, ApiConstants |
...
| Модуль | Папка | Файлов | Зависимости Core |
|--------|-------|--------|-----------------|
| **orders** | `features/orders/` | 8 | BaseHttpService, ApiConstants |
...
| **ai_training** | `features/ai_training/` | 34 | BaseHttpService, ApiConstants |

### Коммуникации
| Модуль | Папка | Файлов | Зависимости Core |
|--------|-------|--------|-----------------|
| **employee_chat** | `features/employee_chat/` | 13 | BaseHttpService, ApiConstants, MediaUploadService, WebSocket |
| **messenger** | `features/messenger/` | 38 | BaseHttpService, ApiConstants, WebSocket |
...
| Модуль | Папка | Файлов | Зависимости Core |
|--------|-------|--------|-----------------|
| **loyalty** | `features/loyalty/` | 16 | BaseHttpService, ApiConstants |
...
| **coffee_machine** | `features/coffee_machine/` | 14 | BaseHttpService, ApiConstants, MultitenancyFilterService, EmployeePushService |
...
| **onboarding** | `features/onboarding/` | 1 | — (только UI, без API) |

---

## 4. Карта зависимостей каждого модуля

### AUTH (Авторизация) — 15 файлов
...
│   ├── pin_entry_page.dart
│   ├── pin_setup_page.dart
│   ├── device_verification_page.dart   ← NEW: Верификация нового устройства
│   └── device_approval_page.dart       ← NEW: Одобрение запросов (developer)
└── widgets/
...
├── POST /api/auth/verify-device-otp          ← NEW: OTP для нового устройства
├── POST /api/auth/request-device-approval    ← NEW: Запрос одобрения
├── GET  /api/auth/device-approval-requests   ← NEW: Список запросов (dev only)
├── POST /api/auth/resolve-device-approval    ← NEW: Одобрить/отклонить
└── GET  /api/auth/device-approval-status/:phone ← NEW: Polling статуса
...
### WORK_SCHEDULE (Рабочий график) — 15 файлов
...
### SHIFT_HANDOVER (Передача смены) — 16 файлов
...
### RECOUNT (Пересчёт товаров) — 26 файлов
...
### AI_TRAINING (ИИ обучение / Vision / YOLO) — 33 файла
...
├── pages/
│   ├── ai_dashboard_page.dart (дашборд ИИ + переключатель эмбеддингов)
...
│   ├── pending_codes_page.dart          # чекбоксы + массовые действия + Autocomplete группы
...
└── widgets/
    ...
    ├── z_report_region_overlay.dart
    └── z_report_region_selector.dart
...
├── loyalty-proxy/api/ai_dashboard_api.js (ИИ дашборд + статистика + обучение + внутренние API)
├── loyalty-proxy/api/yolo_retrain_scheduler.js (авто-ретрейн YOLO)
...
├── loyalty-proxy/modules/z-report-intelligence.js (анализ Z-отчётов)
├── loyalty-proxy/ml/yolo-wrapper.js (Node.js обёртка YOLO + embedding API)
├── loyalty-proxy/ml/yolo_inference.py (Python инференс)
├── loyalty-proxy/ml/yolo_server.py (Python HTTP сервер YOLO + MobileNetV3 embeddings)
├── loyalty-proxy/ml/embedding_catalog.py (каталог эмбеддингов, cosine similarity)
└── loyalty-proxy/ml/build_reference_catalog.py (построение референсного каталога)

Зависит от: BaseHttpService
От него зависят: (изолированный, но связан с shifts через AI верификацию)

Telegram бот (ИИ команды, добавлено 2026-02-23):
├── /ai_status — статус модели, данные, здоровье yolo_server
├── /ai_train — запуск обучения YOLO (с подтверждением)
└── /ai_train_status — статус текущего обучения

Двухступенчатое распознавание 1000+ товаров (добавлено 2026-02-24):
├── Ступень 1: YOLO YOLOv8n (1 класс "pack") → обнаружение пачек
├── Ступень 2: MobileNetV3-Small (576-dim embeddings) → идентификация по cosine similarity
├── Переключатель: useEmbeddingRecognition в app_settings, toggle из AI Dashboard
└── Per-shop readiness: 10 recount + 3 display фото на товар на магазин

Внутренние API (localhost-only, без auth):
├── GET  /api/internal/recount-train-status
└── POST /api/internal/trigger-recount-training

Защита обучения:
├── Бэкап модели (.pt → .pt.backup) перед обучением
├── Автовосстановление из бэкапа при ошибке
├── Авто-retry до 2 попыток с интервалом 5 мин
└── Telegram-уведомления об успехе/ошибке/retry
```

### MESSENGER (Мессенджер) — 39 файлов (36 Flutter + 3 бэкенд)

```
Файлы Flutter:
├── models/
│   ├── chat_folder_model.dart (папки чатов)
│   ├── contact_model.dart
│   ├── conversation_model.dart
│   ├── message_model.dart
│   ├── participant_model.dart
│   └── poll_model.dart (опросы)
├── services/
│   ├── call_service.dart (WebRTC аудиозвонки)
│   ├── messenger_service.dart (HTTP API)
│   ├── messenger_ws_service.dart (WebSocket real-time)
│   └── voice_recorder_service.dart (запись голосовых)
├── pages/
│   ├── messenger_shell_page.dart (обёртка навигации)
│   ├── messenger_list_page.dart (список бесед, debounce WS reload)
│   ├── messenger_chat_page.dart (чат, O(1) reply lookup, in-place reactions)
│   ├── messenger_profile_page.dart (профиль)
│   ├── contact_search_page.dart (поиск контактов)
│   ├── group_create_page.dart (создание группы)
│   ├── group_info_page.dart (информация о группе)
│   ├── call_page.dart (экран звонка)
│   ├── channel_list_page.dart (список каналов)
│   ├── conversation_picker_page.dart (выбор беседы для пересылки)
│   ├── create_poll_page.dart (создание опроса)
│   ├── image_viewer_page.dart (просмотр фото)
│   ├── manage_folders_page.dart (управление папками)
│   ├── photo_editor_page.dart (редактор фото)
│   └── video_note_recorder_page.dart (запись видеосообщений)
└── widgets/
    ├── call_overlay.dart (оверлей звонка)
    ├── chat_list_tile.dart (тайл беседы в списке)
    ├── combined_media_picker.dart (выбор медиа)
    ├── gif_picker.dart (выбор GIF)
    ├── message_bubble.dart (пузырь сообщения)
    ├── message_context_menu.dart (контекстное меню)
    ├── message_input_bar.dart (поле ввода сообщения)
    ├── pinned_message_bar.dart (закреплённые сообщения)
    ├── poll_bubble.dart (пузырь опроса)
    ├── sticker_picker.dart (выбор стикеров)
    └── video_note_player.dart (плеер видеосообщений, LRU cache)

API эндпоинты:
├── GET  /api/messenger/conversations (LATERAL JOIN + batch participants)
├── POST /api/messenger/conversations
├── GET  /api/messenger/conversations/:id/messages
├── POST /api/messenger/conversations/:id/messages (rate limit 30/мин)
├── POST /api/messenger/conversations/:id/read
├── GET  /api/messenger/unread
├── GET  /api/messenger/contacts
├── POST /api/messenger/upload (rate limit 10/мин)
├── POST /api/messenger/groups
├── PUT  /api/messenger/groups/:id
├── GET  /api/messenger/media-stats (dev only)
└── WS   /ws/messenger

Бэкенд файлы:
├── loyalty-proxy/api/messenger_api.js (REST API)
├── loyalty-proxy/api/messenger_websocket.js (WebSocket)
└── loyalty-proxy/api/messenger_media_cleanup_scheduler.js (автоочистка медиа)

DB таблицы: messenger_conversations, messenger_participants (+unread_count),
            messenger_messages, messenger_profiles
DB индексы: idx_msgr_msg_conv_active, idx_msgr_msg_pinned, idx_msgr_part_phone_conv, idx_msgr_conv_type

Зависит от: BaseHttpService, ApiConstants, WebSocket, db.js
От него зависят: (изолированный модуль, замена employee_chat)

ЕСЛИ ИЗМЕНИТЬ MESSENGER:
> WebSocket может разорваться — сообщения не приходят в реальном времени
> Голосовые/видеосообщения могут не записываться/не отправляться
> Звонки (WebRTC) могут не устанавливать соединение
> Rate limiting защищает от спама — не удалять
> unread_count денормализован — при изменении POST /messages проверить инкремент
```
...
### COFFEE_MACHINE (Кофемашины) — 14 файлов
...
│   └── coffee_machine_shop_photos_page.dart (фото магазинов)
└── widgets/
...
### SHOP_CATALOG (Каталог магазина) — 6 файлов

```
Файлы Flutter:
├── models/
│   ├── shop_product.dart
│   └── shop_product_group.dart
├── services/
│   └── shop_catalog_service.dart
├── pages/
│   ├── shop_catalog_admin_page.dart
│   └── shop_catalog_page.dart
└── widgets/
    └── photo_carousel.dart

Бэкенд файлы:
└── loyalty-proxy/api/shop_catalog_api.js

Зависит от: BaseHttpService, shops (данные магазинов)
От него зависят: (изолированный)

ЕСЛИ ИЗМЕНИТЬ:
→ Управление товарами в конкретном магазине перестанет работать
→ Фото-карусель товаров может сломаться
```

### ONBOARDING (Онбординг) — 1 файл

```
Файлы Flutter:
└── pages/
    └── permission_onboarding_page.dart

Бэкенд файлы: (нет — только Flutter UI)

Зависит от: — (нет зависимостей от core сервисов)
От него зависят: main.dart (в цепочке навигации после регистрации)

ЕСЛИ ИЗМЕНИТЬ:
→ Цепочка входа: RegistrationPage → PinSetup → PermissionOnboarding → MainMenu
→ Разрешения (камера, GPS, уведомления) могут не запроситься при первом запуске
```

### SETTINGS (Настройки) — 1 файл

```
Файлы Flutter:
└── pages/
    └── store_links_settings_page.dart    — Настройка ссылок Google Play / App Store

Бэкенд файлы:
└── api/store_links_api.js               — GET/POST /api/app-settings/store-links

Зависит от: BaseHttpService, ApiConstants, app_settings (DB)
От него зависят: main_menu_page.dart (QR-коды в диалоге «Код приглашения»), data_management_page.dart (плитка «Ссылки»)

ЕСЛИ ИЗМЕНИТЬ:
→ QR-коды в диалоге «Код приглашения» перестанут работать
→ Страница настроек ссылок в меню «Управление» сломается
```
...
| Файл | Влияние |
|------|---------|
| `base_http_service.dart` | ВСЕ 38 модулей |
...
| `index.js` (бэкенд роутер) | ВСЕ API эндпоинты (75 файлов) |
...
| ai_training | Только ИИ обучение (но связан с shifts через AI верификацию) |
| messenger | Только мессенджер |
...
| shop_catalog | Только каталог магазина |
| onboarding | Только онбординг (1 файл) |
...
```
Подключает 77 API файлов через require() и setup*()
50+ вызовов setup*API() для маршрутизации
12 планировщиков запущены автоматически
...
WebSocket: employee_chat_websocket + messenger_websocket + counters_websocket
```

### Планировщики (Schedulers) — 12 штук, ВСЕ в api/
...
| 11 | yolo-retrain | `api/yolo_retrain_scheduler.js` | Авто-ретрейн YOLO модели |
| 12 | messenger-media-cleanup | `api/messenger_media_cleanup_scheduler.js` | Очистка медиа мессенджера (>90 дней) |

### Модули бэкенда (modules/) — 9 файлов

| Файл | Назначение |
|------|-----------|
| `modules/counter-ocr.js` | OCR счётчиков кофемашин (вызывает EasyOCR) |
| `modules/ocr-engine.js` | Общий движок OCR |
| `modules/ocr_server.py` | EasyOCR Python сервер (порт 5001) |
| `modules/orders.js` | Push-уведомления о заказах |
| `modules/cigarette-vision.js` | CV распознавание сигарет + бэкап/восстановление + динамический toggle эмбеддингов (useEmbeddingRecognition в app_settings) |
...
| `modules/z-report-intelligence.js` | Анализ и интеллект Z-отчётов |

### ML система (ml/) — 5 файлов

| Файл | Назначение |
|------|-----------|
| `ml/yolo-wrapper.js` | Node.js обёртка для YOLO + embedding API (checkDisplayEmbed, addToCatalog, getCatalogStats) |
| `ml/yolo_inference.py` | Python инференс YOLO модели |
| `ml/yolo_server.py` | Python HTTP сервер YOLO + MobileNetV3-Small embeddings (/display-embed, /embed, /catalog/add, /catalog/stats) |
| `ml/embedding_catalog.py` | Каталог эмбеддингов: cosine similarity поиск, CRUD операции, persistence в JSON |
| `ml/build_reference_catalog.py` | Скрипт построения референсного каталога из обучающих фото |

### Утилиты бэкенда (utils/) — 18 файлов
...
| `utils/db_schema.sql` | SQL схема всех ~44 таблиц PostgreSQL |
...
| `utils/pending_notify.js` | Уведомления о pending-отчётах |
| `utils/push_service.js` | Сервис отправки push-уведомлений |
| `utils/session_middleware.js` | Session middleware (JWT токены) |
| `utils/test_file_lock.js` | Тест блокировки файлов |
| `utils/id_generator.js` | Генерация уникальных ID |
| `utils/s3_storage.js` | Клиент Yandex S3 облачного хранилища |
| `utils/video_compress.js` | Сжатие видео |

### Admin Bot (admin-bot/) — Telegram бот администратора

| Файл | Назначение |
|------|-----------|
| `admin-bot/index.js` | Telegram бот: OTP, SSL, **AI команды** (/ai_status, /ai_train, /ai_train_status) |

Бот вызывает внутренние API (localhost:3000) без авторизации:
- `GET /api/internal/recount-train-status` — статус обучения
- `POST /api/internal/trigger-recount-training` — запуск обучения
- `GET http://127.0.0.1:5002/health` — здоровье yolo_server

### Сервисы бэкенда (services/) — 1 файл

| Файл | Назначение |
|------|-----------|
| `services/telegram_bot_service.js` | Telegram бот для OTP |

### Полный список API файлов (77 штук)

**Auth & Users (4):** auth_api, employees_api, employee_registration_api, shop_managers_api
**Shops (5):** shops_api, shop_settings_api, shop_coordinates_api, shop_products_api, shop_catalog_api
...
**AI/Vision (7):** cigarette_vision_api, z_report_api, master_catalog_api, master_catalog_notifications, ai_dashboard_api, yolo_retrain_scheduler
**Messenger (2):** messenger_api, messenger_websocket
**Chat (3):** employee_chat_api, employee_chat_websocket, media_api
**Clients (1):** clients_api
**Loyalty (4):** loyalty_gamification_api, loyalty_promo_api, loyalty_wallet_api, bonus_penalties_api
**WebSocket (1):** counters_websocket
**Health (1):** health_monitor_api
**Settings (1):** store_links_api
**Other (17):** referrals_api, review_api, recipes_api, suppliers_api, product_questions_api, product_questions_notifications, product_questions_penalty_scheduler, tasks_api, recurring_tasks_api, job_applications_api, data_cleanup_api, execution_chain_api, geofence_api, pending_api, report_notifications_api, withdrawals_api, coffee_machine_api + coffee_machine_automation_scheduler

### PostgreSQL (основное хранилище с 2026-02-17)

```
База: arabica_db, пользователь: arabica_app, peer auth
Таблиц: ~44, Записей: 10400+, Размер: 17MB
...
Паттерн dual-write:
  ...
  47 feature flag (USE_DB_*=true) в pm2 env

ЕСЛИ ИЗМЕНИТЬ db.js:
> Все 47 модулей с DB потеряют доступ к данным
...
### Хранилище данных (/var/www/) — 113+ директорий (backup/fallback)
...
│   ├── z-report-samples/
│   ├── master-catalog/
│   ├── counting-training/ (YOLO обучающие данные)
│   └── messenger-media/ (медиа мессенджера)
│
├── OTHER
│   ├── job-applications/
│   ├── suppliers/
...
### shared/ — Общие виджеты и модели (18 файлов)
...
### app/ — Навигация и общие страницы (12 файлов)
...
└── pages/ (9 файлов)
    ...
    ├── developer_reports_page.dart (отчёты разработчика)
    ├── staff_efficiency_page.dart (эффективность персонала)
