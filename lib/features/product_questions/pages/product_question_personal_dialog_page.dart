...
class ProductQuestionPersonalDialogPage extends StatefulWidget {
  ...
  const ProductQuestionPersonalDialogPage({
    super.key,
...
class _ProductQuestionPersonalDialogPageState extends State<ProductQuestionPersonalDialogPage> {
  ...
  Future<void> _loadData() async {
    ...
    // Отмечаем как прочитанный для клиента
    ...
    return Align(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            // Для сообщений сотрудников показываем имя
            if (!isClientMessage && message.senderName != null) ...[
              Text(
                message.senderName!,
                style: TextStyle(
                  fontSize: 12,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Поиск товара - ${widget.shopAddress}'),
