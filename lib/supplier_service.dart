...
class SupplierService {
  ...
  static const String baseUrl = '$serverUrl/api/suppliers';
  ...
  static Future<Supplier?> getSupplier(String id) async {
    try {
      ...
      if (response.statusCode == 200) {
        final result = jsonDecode(response.body);
        if (result['success'] == true) {
          return Supplier.fromJson(result['supplier'] as Map<String, dynamic>);
        }
      }
      return null;
    } catch (e) {
  ...
  static Future<Supplier?> createSupplier(Supplier supplier) async {
    try {
      ...
      if (response.statusCode == 200) {
        final result = jsonDecode(response.body);
        if (result['success'] == true) {
          Logger.debug('✅ Поставщик создан: ${result['supplier']['id']}');
          return Supplier.fromJson(result['supplier'] as Map<String, dynamic>);
        } else {
        ...
        Logger.error('   Тело ответа: $errorBody');
      }
      return null;
    } catch (e) {
  ...
  /// Обновить поставщика
  static Future<Supplier?> updateSupplier(String id, Supplier supplier) async {
    try {
      Logger.debug('📝 Обновление поставщика: $id');
      ...
      if (response.statusCode == 200) {
        final result = jsonDecode(response.body);
        if (result['success'] == true) {
          Logger.debug('✅ Поставщик обновлен: $id');
          return Supplier.fromJson(result['supplier'] as Map<String, dynamic>);
        } else {
  ...
  static Future<bool> deleteSupplier(String id) async {
    try {
      ...
      if (response.statusCode == 200) {
        final result = jsonDecode(response.body);
        if (result['success'] == true) {
          Logger.debug('✅ Поставщик удален: $id');
          return true;
        } else {
