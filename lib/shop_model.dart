  ...
  static Future<List<Shop>> loadShopsFromGoogleSheets() async {
    try {
      ...
      if (maxRows < targetRows) {
        print('');
        print('⚠️ ВАЖНО: Google Sheets CSV экспорт не возвращает пустые строки!');
        print('   Если в таблице есть пустые строки между данными, они не будут включены в CSV.');
        print('   Для получения всех 800 строк убедитесь, что:');
        print('   1. В таблице действительно есть данные в столбце D до строки 800');
        print('   2. Или используйте Google Sheets API вместо CSV экспорта');
        print('');
      }
